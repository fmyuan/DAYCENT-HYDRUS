...
c ..... Fill in the date parameters
        ...
        timary(ttlind, 3) = readint(15,buffr,lch,istat)
c ..... The Julian day is read in in place of the month, determine
c ..... the month that corresponds to the Julian day, cak - 04/17/03
        timary(ttlind, 2) = jday2month(timary(ttlind, 3))

c ..... check for the explicit month argument
        if (istat.eq.0) then
          if (ttlind.gt.1) then
            timary(ttlind, 1) = timary(ttlind-1, 1)
            timary(ttlind, 3) = yrnum
c ......... The Julian day is read in in place of the month, determine
c ......... the month that corresponds to the Julian day, cak - 04/17/03
            timary(ttlind, 2) = jday2month(timary(ttlind, 3))
c ....... bad input
...
c ... Determine which month the Julian falls within
      integer function jday2month (jday)
...
c ... Local variables
      integer jday_start(12), jday_end(12), month
      data jday_start /1, 32, 60, 91, 121, 152, 182, 213, 244, 274,
     &                 305, 335/
      data jday_end /31, 59, 90, 120, 151, 181, 212, 243, 273, 304,
     &               334, 365/

      if (jday .gt. 365 .or. jday .le. 0) then
        write(*,*) "Invalid Julian date in schedule file!"
        write(*,*) "Julian day = ", jday
        STOP
      endif

      do 10 month = 1, 12
        if (jday .ge. jday_start(month) .and.
     &      jday .le. jday_end(month)) then
          jday2month = month
          goto 999
        endif
10    continue

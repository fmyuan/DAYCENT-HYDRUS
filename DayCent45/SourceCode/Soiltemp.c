...
**                      year (degrees C)
**    sitepar->timlag - time lag in Julian days from the beginning of the year
...
**    arrayindx      - position in the circular temparray array
...
**    temparray[]    - array used to store the average soil temperatures in
...
#include "stemp.h"
    ...
    void soiltemp(int jday, float biomass, float tmin, float tmax,
                  ...
                  float tmns, float tmxs, float *soiltavewk)
    {
...
/*          diff1=-depth[ii]* (float)pow((.001/(double)avtd),0.5); */
/*          diff1=-depth[ii]* (float)pow((0.0000364/(double)avtd),0.5); */
      ...
      /* temparray using the soil temperature value calculated on the first */
      /* day of the run, cak - 04/27/04 */
      if (startofrun) {
        for (arrayindx = 0; arrayindx < 7; arrayindx++) {
          temparray[arrayindx] = soiltavg[1];
        }
      ...
      *soiltavewk = 0.0f;
      for (ii = 0; ii < 7; ii++) {
        *soiltavewk = *soiltavewk + temparray[ii];
      }

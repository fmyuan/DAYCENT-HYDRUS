...
c ..... Determine C/E ratios for flows to som2
     ...
     &             ((som1c(1)/som1e(1,iel))-pcemic1(2,iel))
...
c        tcflow = som1c(SRFC) * defac * dec3(SRFC) * dtm
        tcflow = som1c(SRFC) * agdefac * dec3(SRFC) * dtm * pheff
c ..... where
...
c ..... Determine C/E ratios for flows to som2
        do 30 iel=1,nelem
          rceto2(iel) = bgdrat(aminrl,varat21,iel)
30      continue
...
c     &           anerb * dtm
        tcflow = som1c(SOIL) * bgdefac * dec3(SOIL) * cltfac(1) *
     &           eftext * anerb * dtm * pheff
c ..... where
...
c     &           dtm
        tcflow = som2c(SOIL) * bgdefac * dec5(SOIL) * cltfac(2) *
     &           anerb * dtm * pheff
c ..... where
...
c ....... Changed csrsnk to s2c2 (10/92)
          call respir(co2los,2,SOIL,som2c,som2ci,s2c2,resp,
...
c ....... accumulators.
          do 80 iel=1,nelem
            ...
            call esched(cfs2s3,som2c(SOIL),rcetob,som2e(SOIL,iel),
...
c ....... accumulators.
          do 90 iel=1,nelem
            call esched(cfs2s1,som2c(SOIL),rceto1(iel),som2e(SOIL,iel),
...
c        tcflow = som2c(SRFC) * defac * dec5(SRFC) * dtm
        tcflow = som2c(SRFC) * agdefac * dec5(SRFC) * dtm * pheff
c ..... where
c .....   dec5(SRFC) = intrinsic decomposition rate of soil som2
...
c ....... Changed csrsnk to s2c2 (10/92)
          call respir(co2los,2,SRFC,som2c,som2ci,s2c2,resp,
     &                som2e,minerl,gromin,s2mnr,newminrl)
...
c        tcflow = som3c * defac * dec4 * cltfac(3) * anerb * dtm
        tcflow = som3c * bgdefac * dec4 * cltfac(3) * anerb * dtm *
     &           pheff
c ..... where
...
c ..... Compute and schedule N, P, and S flows 
     ...
     &                som2e(SRFC,iel),som2e(SOIL,iel),minerl(SRFC,iel),
     &                mnrflo)
120     continue
      endif

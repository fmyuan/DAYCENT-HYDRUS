...
**    tadep                 - depth in the soil profile above which
**                            transpiration occurs (cm)
**
**  LOCAL VARIABLES:
...
/*!!*/
      if (sitepar->usexdrvrs) {
        snowmodel(*jday, *tempmax, *tempmin, *avgtemp, -1.0f, *pptactual,
                  ...
                  sitepar->sublimscale, petleft, tmelt, snlq, *month);
      } else {
        snowCent(tmelt, *avgtemp, *pptactual, &pptsoil, snowpack, snlq,
                 &petleft, melt, accum, sublim, *tempmin, *tempmax, *month,
                 sitepar->rlatitude, *jday);
      }
      ...
      *intrcpt = wintstcr + wintlit;
      ...
      if (*snowpack > 0) {
        ...
        potbse(&bserate, layers->nelyrs, layers->sumecoeff, layers->ecoeff,
               totagb, fbse, petleft, layers->width, layers->swc, layers,
               *biolive);
      }
      ...
      *pottransp = bstrate;
      ...
      h2oflux(*jday, layers->numlyrs, layers->swc, layers->swcmin,
              ...
              *snowpack, runoffdly, basef, baseflow,
              sitepar->watertable[*month], sitepar->hpotdeep,
              sitepar->ksatdeep);
...
/*      printf("petleft = %8.4f\n", petleft); */
      ...
      if (petleft > 0) { 
        ...
        /* Transpiration occurs in only the top Century nlaypg layers. */
        /* Compute the number of DayCent layers are in the top Century */
        /* nlaypg layers.  Pass this value to the soiltransp subroutine */
        ...
        soiltransp(layers->swc, transp, tranlayers, layers->tcoeff,

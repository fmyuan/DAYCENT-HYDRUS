...
c                       All Rights Reserved
      ...
      subroutine treegrow (tfrac,tavewk)
...
c ... Local variables
      ...
      real      cprodfwk, eprodfwk(MAXIEL)
      character subname*10
C -----------fmyuan: modification for BIOCOMPLEXITY Project ------------
c ... the time when living forest C very low (i.e., seedlings) 
	INTEGER	 time0
      common /misc/ time0
C -----------fmyuan: modification for BIOCOMPLEXITY Project ------------
...
C -----------fmyuan: modification for BIOCOMPLEXITY Project ------------
c ... old or new forest should be upon tree age RATHER THAN time itself
      if ((int(time)-time0) .le. swold) then
C -----------fmyuan: modification for BIOCOMPLEXITY Project ------------
C      if (time .le. swold) then
...
c ..... and leaves, cak - 07/02/02
     ...
     &              pforc, rimpct, forstg, snfxmx(FORSYS), cprodfwk,
     &              eprodfwk, uptake, tree_a2drat, treeNfix, relyld)
      else
        cprodfwk = 0.0
      endif

c ... If growth occurs...
      if (cprodfwk .gt. 0.) then

c ..... Compute carbon allocation fractions for each tree part
c ..... Calculate how much of the carbon the roots use
        cprodfLeft = cprodfwk - (cprodfwk * tree_cfrac(FROOT))
...
c ..... Update accumulators for N, P, and S uptake
        do 85 iel = 1, nelem
          eupacc(iel) = eupacc(iel) + eprodfwk(iel)
...
c ..... C/N ratio for production
        if (eprodfwk(N) .eq. 0.0) then
          write(*,*) 'Error in treegrowth, eprodfwk(N) = 0.0)'
          STOP
        endif
        tcnpro = cprodfwk/eprodfwk(N)
...
c ..... Added mrspwkflow variable for weekly timestep, cak - 08/07/02
        do 95 ipart = 1, FPARTS
          mfprd(ipart) = tree_cfrac(ipart) * cprodfwk
...
c ..... Actual Uptake
        do 110 iel = 1, nelem
          if (eprodfwk(iel) .eq. 0.0) then
            write(*,*) 'Divide by zero in treegrow, eprodfwk(iel) = 0'
            STOP
          endif
          euf(LEAF) = eup(LEAF,iel) / eprodfwk(iel)
          euf(FROOT) = eup(FROOT,iel) / eprodfwk(iel)
          euf(FBRCH) = eup(FBRCH,iel) / eprodfwk(iel)
          euf(LWOOD) = eup(LWOOD,iel) / eprodfwk(iel)
          euf(CROOT) = eup(CROOT,iel) / eprodfwk(iel)
...
c ... Sum the weekly production variables for output to the monthly *.bin file
      cprodf = cprodf + cprodfwk
      do 160 iel = 1, nelem
        eprodf(iel) = eprodf(iel) + eprodfwk(iel)
160   continue

...
c ..... Added frtcindx and frtc(4) for dynamic-C allocation -mdh 8/23/00
        ...
        frtcindx = int(temp)
        if (frtcindx .lt. 0 .or. frtcindx .gt. 2) then
...
40      continue
        ...
        if ((frtc(1) .lt. frtc(2))) then
          write(*,*) 'frtc(1) must be >= frtc(2) in crop.100' 
          write(*,*) 'frtc(1) = ', frtc(1)
          write(*,*) 'frtc(2) = ', frtc(2)
          STOP
        endif
        if ((frtc(3) .le. 0.0) .and. (frtcindx .eq. 2)) then
          write(*,*) 'frtc(3) must be > 0.0 when frtcindx=2'
          write(*,*) 'frtc(3) = ', frtc(3)
          STOP
        endif
        if ((frtcindx .eq. 2) .and.
     &      (frtc(4) .lt. 0.0 .or. frtc(4) .ge. 1.0)) then
          write(*,*) 'frtc(4) must be >= 0.0 and < 1.0 when frtcindx=2'
          write(*,*) 'frtc(4) = ', frtc(4)
          STOP
        endif

...
**                            1 = on
**    frac_nh4_fert         - the fraction of N fertilizer that is NH4+
**    frac_no3_fert         - the fraction of N fertilizer that is NO3-
**                            (frac_nh4_fert + frac_no3_fert = 1.0)
...
FILES_SPT files = &filestruct;
                ...
                double *frac_no3_fert, int *texture)
    {
      ...
      *frac_nh4_fert = sitepar->frac_nh4_fert;
      *frac_no3_fert = sitepar->frac_no3_fert;
      *texture = sitepar->texture;

/* -----------fmyuan: modification for BIOCOMPLEXITY Project ------------*/
/*     initialization of hydrus running controls                         */
	  hydrus_init(sitepar->ifhydrus, swcinit);
/* -----------fmyuan: modification for BIOCOMPLEXITY Project ------------*/
      ...
      *numlyrs = layers->numlyrs;
      ...
      if (files->write_wb) {
        ...
        fprintf(files->fp_wb, " %7s %9s %9s %9s\n", "outflow", "balance", "streamflow","runoff");
      }
      ...
      if (files->write_yearsum) {
        ...
        fprintf(files->fp_yearsum, "%6s  %10s  %10s  %10s\n", "time",
                "N2Oflux", "NOflux", "CH4");
      }
      ...
      if (files->write_livec) {
        ...
        fprintf(files->fp_livec, "%10s  %10s  %10s %10s %10s\n",
                "fbrchc", "rlwodc", "crootc","lai", "vegcover");
      }
      ...
      if (files->write_soilc) {
        ...
        fprintf(files->fp_soilc, "%10s  %10s\n",
                "som2c", "som3c");
      }
      ...
      if (files->write_sysc) {
        ...
        fprintf(files->fp_sysc, "%6s  %2s  %10s  %10s  %10s  %10s\n", "time",
                "wk", "livec", "deadc", "soilc", "sysc");
      }

...
c                       All Rights Reserved
      ...
      subroutine potcrp (month, cancvr, irractwk, tminwk, tmaxwk,
     &                   pptwk, petwk, tfrac, jdaywk)
...
c ... Argument declarations
      ...
      real    cancvr
      real    irractwk, tminwk, tmaxwk, pptwk, petwk, tfrac
...
c ... Local variables
     ...
     &         bioprd, ctemp, potprd, ratlc, rtsh, sdlng,
...
c        h2ogef(1) = (avh2o(1) + pptwk + irractwk)/petwk
        h2ogef(1) = (avh2o(1) + ((pptwk+irractwk)/tfrac))/(petwk/tfrac)
      else
        h2ogef(1) = 0.01
      endif
...
c ..... Calculate potential production (biomass)
        agprod = shwave(month,sitlat,jdaywk) * prdx(1) * potprd *
...
c ..... production, cak - 09/09/02
        if (agprod .gt. 0.0) then
          call cropDynC(agprod, rtsh)
        else
          agprod = 0.0
        endif

...
!      parameter (TREELNS = 120)
      parameter (TREELNS = 132)
C -----------fmyuan: modification for BIOCOMPLEXITY Project ------------
...
c ..... frfrac(2) Added for dynamic allocation. -mdh 11/15/00
        read(11,*) frfrac(1), name
        call ckdata('treein', 'frfrac', name)
        read(11,*) frfrac(2), name
        call ckdata('treein', 'frfrac', name)

        if ((frfrac(1) .eq. 0.0) .or. (frfrac(2) .eq. 0.0)) then
          write(*,*) 'frfrac(1) and frfrac(2) in tree.100 must be > 0.0'
          write(*,*) 'frfrac(1) = ', frfrac(1)
          write(*,*) 'frfrac(2) = ', frfrac(2)
          STOP
        endif
        if ((frfrac(1) .gt. frfrac(2))) then
          write(*,*) 'frfrac(1) must be <= frfrac(2) in tree.100'
          write(*,*) 'frfrac(1) = ', frfrac(1)
          write(*,*) 'frfrac(2) = ', frfrac(2)
          STOP
        endif
...
C -----------fmyuan: modification for BIOCOMPLEXITY Project ------------
        do 200 ii = 1, MONTHS
          read(11, *) twigdr(ii), name
          call ckdata('treein','twigdr',name)
200     continue
C -----------fmyuan: modification for BIOCOMPLEXITY Project ------------

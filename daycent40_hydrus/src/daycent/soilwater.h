...
typedef struct
{
  ...
  double frac_nh4_fert;
  double frac_no3_fert;
  int    texture;
/* -----------fmyuan: modification for BIOCOMPLEXITY Project (June 2008) ---*/ 
  int    ifhydrus;   /* HYDRUS 1D switch off/on (0/1) */       
...
void h2oflux(int jday, int numlyrs, double swc[MAXLYR], double swcmin[MAXLYR],
             ...
             float snowpack, int watertable, float hpotdeep, float ksatdeep);
...
void potbse(float *bserate, int nelyrs, float sumecoeff, float ecoeff[MAXLYR],
            ...
            double swc[MAXLYR], LAYERPAR_SPT layers);
...
/*void watrflow(int *jday, int *month, int *nlayer, int *nlaypg,
              ...
              float *snlq, float amovdly[CENTMAXLYR], float fwloss[4],
              float *pet, float *evaptot, float *trantot, float *stream1,
              float *stormf, float *basef, float *pottransp, float *stormflow,
              float *baseflow, float *accum, float *melt, float *intrcpt,
              float *outflow, float tmelt[], float *sublim, float wfluxout[],
              float *time, float *strplt, float *co2val,
              float *tmns, float *tmxs,float *runoffdly);*/
...
void soiltemp(int jday, float biomass, float tmin, float tmax,
              ...
              float rlat, float tmns, float tmxs);

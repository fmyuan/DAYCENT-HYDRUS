...
#include <stdlib.h>
                      ...
                      char* subname, int subnamelen)
    {
      ...
      double cum_no3 = 0.0;
      double cum_tot = 0.0;
      double no3_clyr_psum = 0.0;
//    char   subname[10];

      extern LAYERPAR_SPT layers;

      //strcpy(subname, nameid);
      subname[subnamelen-1] = '\0';

      if (fabsf(*amt) < 1.0E-30) {
        return;
      }
...
//       fprintf(stdout, "Error in update_npool: %s.  ",
//               subname);
//        fprintf(stdout, "no3amt = %12.10f,  cum_no3 = %12.10f\n", no3amt, cum_no3); 
//        exit(1);
      }
